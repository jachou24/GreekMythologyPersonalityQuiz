...
let scores = new Map();
for (k = 0; k < gods.length; k++) {
    scores.set(gods[k], {val: 0});
}

const resultImgList = [".png", ".png", ".png", ".png", ".png", ".png", ".png", ".png", ".png"]
let resultsImgMap = new Map();
for (m = 0; m < gods.length; m++) {
    resultsImgMap.set(gods[m], resultImgList[m]);
}
...
function showResults(keyOfHighestValue) {
    resultImg = resultsImgMap[keyOfHighestValue];
...
function newQuestion(questionNumber, butid) {
   // update scores
    ...
    // clearing prompts from the container
    const qDiv = document.getElementsByClassName('qDiv');
    qDiv.innerHTML = '';
